...
target : source1 source2 ...
	commands to make target (a.k.a. recipes)
```
...
A _target_ is usually the name of a file generated by make when it automatically recompiles all specified source files that have changed. However, it can also represent an action that **make** will carry out directly. In that case, the typical syntax of a _makefile_ may look as follows: 
...
In this case, **make** will delete files 'test1' and 'test2' directly, without checking for their _mtime_ flags.

We now illustrate how to write a _makefile_ and use **make** with the following simple project, which consists of two source files, named 'test1.C' and 'test2.C'. The content of 'test1.C' is:
...
In the _makefile_, we write the following content:
...
ls -alt test1
-rwxr-xr-x 1 abilandz abilandz 15960 Oct 11 14:33 test1
```

However, if we re-execute the same command, nothing will happen:
...
We can either change something in the source code of 'test1.C', or simply directly update its _mtime_ flag with the **touch** command:
...
compilation of test1 succeeded
ls -alt test1
-rwxr-xr-x 1 abilandz abilandz 15960 Oct 11 14:39 test1
```
...
```bash
$ touch test1.C # update mtime flag directly
$ make --silent test1
compilation of test1 succeeded
-rwxr-xr-x 1 abilandz abilandz 15960 Oct 11 14:36 test1
```

Alternatively, if we want to silence differentially only specific commands, we can prefix only those command with ```@```  in the _makefile_:
...
With such a modified _makefile_, we get:
...
gcc test1.C -o test1 && echo "compilation of test1 succeeded"
compilation of test1 succeeded
-rwxr-xr-x 1 abilandz abilandz 15960 Oct 11 14:47 test1
```

The line ```ls -alt test1``` is now not shown in the printout, because we have silenced it by prepending ```@``` in the _makefile_.

To compile both 'test1.C' and 'test2.C' we proceed as follows:

```bash
$ touch test1.C test2.C # update mtime flag directly
$ make all
gcc test1.C -o test1 && echo "compilation of test1 succeeded"
compilation of test1 succeeded
-rwxr-xr-x 1 abilandz abilandz 15960 Oct 11 14:50 test1
gcc test2.C -o test2 && echo "compilation of test2 succeeded"
...
gcc test1.C -o test1 && echo "compilation of test1 succeeded"
compilation of test1 succeeded
-rwxr-xr-x 1 abilandz abilandz 15960 Oct 11 14:51 test1
```

As we can see, only the object file 'test1' was recompiled, because only the _mtime_ flag of the source code 'test1.C' has changed!
...
As we can see, only the object file 'test2' was recompiled, because only the _mtime_ flag of source code 'test2.C' has changed!

We have specified two additional actions in the makefile: **run** and **clean**. For the first one, we have in the _makefile_ the following definition:
...
./test1 && ./test2
 ...
 Hi there, from test1! Compilation time was: on Oct 11 2025 at 14:51:34
...
And finally, to clean up the object files, and recompile everything from scratch:

```bash
# List the content of current working directory:
$ ls 
makefile  test1  test1.C  test2  test2.C

# Remove rhe object files:
$ make clean
rm test1 test2

# List again the content of current working directory:
$ ls
makefile  test1.C  test2.C
```
